...
######################################################################
def SummarizedGenes (cgapcgi, base, request):
  ...
  partition = BackEnd('',0,cgapcgi,base,'GetPartition.pl','',"'ORG','SCOPE','TITLE','TYPE','TISSUE','HIST','PROT','SORT'",request,{'ORG':org,'SCOPE':scope,'TITLE':title,'TYPE':type,'TISSUE':tissue,'HIST':hist,'PROT':prot,'SORT':sort})
  ...
  if (re.search("Error", partition) != None):
    return partition

  
  genes = BackEnd('',0,cgapcgi,base,'ListSummarizedGenes.pl','',"'ROW','WHAT','ORG','SCOPE','TITLE','TYPE','TISSUE','HIST','PROT','SORT','PARTITION'",request,{'ROW':row,'WHAT':what,'ORG':org,'SCOPE':scope,'TITLE':title,'TYPE':type,'TISSUE':tissue,'HIST':hist,'PROT':prot,'SORT':sort,'PARTITION':partition})
...
######################################################################
def FindSummary (cgapcgi, base, request):
  ...
  response = BackEnd('',0,cgapcgi,base,'GetSummaryTable.pl','',"'ORG','SCOPE','TITLE','TYPE','TISSUE','HIST','PROT','SORT','PARTITION'",request,{'ORG':org,'SCOPE':scope,'TITLE':title,'TYPE':type,'TISSUE':tissue,'HIST':hist,'PROT':prot,'SORT':sort,'PARTITION':partition})
  ## cmd = binpath + "/" + "GetSummaryTable.pl " + \
  ##   "'" + str(base)     + "' " + \
  ##   "'" + str(org)      + "' " + \
  ##   "'" + str(scope)    + "' " + \
  ##   "'" + str(title)    + "' " + \
...
######################################################################
def FindXProfiledGenes (cgapcgi, base, cache, page, org, row, what, request):

  import re

  genes = BackEnd('',0,cgapcgi,base,'ListXProfiledGenes.pl','',"'CACHE','PAGE','ORG','ROW','WHAT'",request,{'CACHE':cache,'PAGE':page,'ORG':org,'ROW':row,'WHAT':what})
...
######################################################################
def FindXProfile (cgapcgi, base, org, form, request):
  import string
  ...
  if (a_set == [] and b_set == []):
    ...
    return BackEnd('',0,cgapcgi,base,'GetXProfile.pl','',"'ORG','A_SET','B_SET'",request,{'ORG':org,'A_SET':str(string.join(a_set, ',')),'B_SET':str(string.join(b_set, ','))})
...
######################################################################
def FindLibsForXProfiler (cgapcgi, base, request):
  import string
  import re
  ...
  libs = BackEnd('',0,cgapcgi,base,'GXSLibrarySelect.pl','',"'ORG','SCOPE','SEQS','SORT','TITLE_A','TITLE_B','TYPE_A','TYPE_B','TISSUE_A','TISSUE_B','HIST_A','HIST_B','PROT_A','PROT_B','COMP_A','COMP_B'",request,{'ORG':org,'SCOPE':scope,'SEQS':seqs,'SORT':sort,'TITLE_A':title_a,'TITLE_B':title_b,'TYPE_A':type_a,'TYPE_B':type_b,'TISSUE_A':tissue_a,'TISSUE_B':tissue_b,'HIST_A':hist_a,'HIST_B':hist_b,'PROT_A':prot_a,'PROT_B':prot_b,'COMP_A':comp_a,'COMP_B':comp_b})
