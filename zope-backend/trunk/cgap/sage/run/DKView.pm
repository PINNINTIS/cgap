...
sub GetDKMicrobeByTags_1 {
  ...
  $sql =
    "select unique TAG, TYPE, ACCESSION, GENOME_DESCRIPTION, START_POSITION, STRAND, GENOME_SIZE " .
    "from  $CGAP_SCHEMA.DK_BACTERIAL_VIRAL " .
    "where TAG in ( $list )"; 
  $stm = $db->prepare($sql);
  if(not $stm) {
    ## print STDERR "$sql\n";
  ...
  while (my ($TAG, $TYPE, $ACCESSION, $GENOME_DESCRIPTION, $START_POSITION, $STRAND, $GENOME_SIZE) = $stm->fetchrow_array()) {
     $ACCESSION =~ s/\.\d+//;
     push @lines, 
       "<TR><TD>" . (join "</TD><TD>",
                      $TAG,
...
#        "<td><font color=\"white\"><b>Accession</b></font></td>" .
        "<td><font color=\"white\"><b>Genome Description</b></font></td>" .
        "<td nowrap><font color=\"white\"><b>Start Position</b></font></td>" .
        "<td><font color=\"white\"><b>Strand</b></font></td>" .
        "<td nowrap><font color=\"white\"><b>Genome Size</b></font></td>" .
        "</tr>";
    push @results, "<BR><BR><BR><center>" ;
    push @results, $TABLE_HEADER ;
...
######################################################################
sub GetBatchDKMicrobe_1 {
  my ($base, $page, $filedata) = @_;
  ...
  my $DK_BACTERIAL_VIRAL_file = CACHE_DK_EXIST_ROOT . "DK_BACTERIAL_VIRAL.dat";
  open(IN, "$DK_BACTERIAL_VIRAL_file") or die "Can't open $DK_BACTERIAL_VIRAL_file.";
  while(<IN>) {
    chop;
    my ($TYPE, $ACCESSION, $GENOME_DESCRIPTION, $START_POSITION, $STRAND, $GENOME_SIZE, $TAG, $FREQ) = split "\t", $_; 
    $ACCESSION =~ s/\.\d+//;
    $tag_type{$TAG}{$TYPE}{$START_POSITION} = 
                join "\t", $ACCESSION, $GENOME_DESCRIPTION, $STRAND, $GENOME_SIZE;
  }
  ...
  for (my $t = 0; $t < @tempArray; $t++ ) {
    ...
    if ($tempArray[$t] eq "") {
      ...
      if ( defined $tag_type{$tempArray[$t]} ) {
        for my $type ( keys %{$tag_type{$tempArray[$t]}} ) {
          for my $START_POSITION ( keys %{$tag_type{$tempArray[$t]}{$type}} ) {
            my ($ACCESSION, $GENOME_DESCRIPTION, $STRAND, $GENOME_SIZE) =      
               split "\t", $tag_type{$tempArray[$t]}{$type}{$START_POSITION};
            push @good_lines,
               "<TR><TD>" .  (join "</TD><TD>",
                               $tempArray[$t],
                               $type,
##                               $ACCESSION,
  ...
  ## GetDataFromOracle(\@all_lines, \%tag2allinfo);
...
sub GetDataFromOracle {
  my ($all_lines_ref, $tag2allinfo_ref) = @_;
  ...
  for(my $i = 0; $i < @all_lines; $i += ORACLE_LIST_LIMIT) {
    ...
    $sql =
       "select unique TAG, TYPE, ACCESSION, START_POSITION, GENOME_SIZE " .
       "from  $CGAP_SCHEMA.DK_BACTERIAL_VIRAL " .
       "where TAG in ( $list )";
    ...
    while ( my ($TAG, $TYPE, $ACCESSION, $START_POSITION, $GENOME_SIZE ) = 
                                                   $stm->fetchrow_array()) {
      $ACCESSION =~ s/\.\d+//;
      $$tag2allinfo_ref{$TAG}= "<TR><TD>" .  (join "</TD><TD>", $TAG, $TYPE, $ACCESSION, $START_POSITION, $GENOME_SIZE) . "</TR></TD>";
    }
  }
...
sub Make_detail_html {   
  ...
  $lines_2 = $lines_2 .
    ...
    "</td></tr>\n" .
    "<tr>" .
    "<td colspan=2 bgcolor=\"38639d\" align=center><font color=\"FFFFFF\">\n" .
