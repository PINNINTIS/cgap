...
sub UploadExperimentalFile_1 {
  ...
  push @lines, "<form name=\"dkdownloadform\" action=\"DKDownload\" method=post target=_blank>\n" .
    ...
    "<B>Download mapped tags with non zero frequence</B>\n" .
    "</font></td></tr><br>\n" .
        "<tr><td>&nbsp;\n" .
    "</td>\n" .
    "<td>&nbsp;\n" .
    ...
    "<tr>" .
    "<td colspan=2 bgcolor=\"336699\" align=center><font color=\"FFFFFF\">\n" .
    "<B>Enter window size for smooting data and get DKView</B>\n" .
    "</font></td></tr><br>\n" .
        "<tr><td>&nbsp;\n" .
    "</td>\n" .
    "<td>&nbsp;\n" .
...
sub DKDownload_1 {
  ...
  print "Tag\tChr\tPosition\tStrand\tFreq\n";
  while (<IN>) {
    chop;
    my ($tag, $chr, $pos, $strand, $tag_num, $freq) = split /\t/, $_;
    if( $freq != 0 ) {
      print join ("\t", $tag, $chr, $pos, $strand, $freq) . "\n";
    }
  }
...
sub DKView_1 {
  ...
  for my $chr (@chr_order) {
    for (my $i = 0; $i < @{ $window_value{$chr} }; $i++) {
      ...
      push @window_info, "$mapped_value\t$f\t$value\n";
      my $high_value = sprintf("%.2f", $high_window_value{$chr}[$i]);
      push @high_window_info, "$mapped_value\t$f\t$high_value\n";
      my $low_value = sprintf("%.2f", $low_window_value{$chr}[$i]);
      push @low_window_info, "$mapped_value\t$f\t$low_value\n";
    }
  }
  ...
  undef $data;
  $data = join "", @high_window_info;
  my $high_window_info_cache_id = WriteDKDataToCache($data);
  if( $high_window_info_cache_id == 0 ) {
    return "There is a error in the DKView process.";
  }

  undef $data;
  $data = join "", @low_window_info;
  my $low_window_info_cache_id = WriteDKDataToCache($data);
  if( $low_window_info_cache_id == 0 ) {
    return "There is a error in the DKView process.";
  }
  ...
  return pick_nth_tag($window_info_cache_id, $bin_size, $window_size, 
                      $org, $filename, 
                      $high_window_info_cache_id, $high_window_size,
                      $low_window_info_cache_id, $low_window_size);
}
...
#    average of window-averaged values
  ...
  my ($window_info_cache_id, $bin_size, $window_size, $org, $filename, 
      $high_window_info_cache_id, $high_window_size,
      $low_window_info_cache_id, $low_window_size) = @_;
...
##
  ...
  for (my $i = 1; $i < @{ $binned_data{$chr} } - 1; $i++ ) {
    ...
    push @{ $image_map_ref },
      ...
      "href=javascript:spawn(\"DKViewDetail?CHR=$chr&POS=$i&CACHE_ID=$window_info_cache_id&WINDOWSIZE=$window_size&FILENAME=$filename&BINNED_DATA_CACHE_ID=$binned_data_id&HIGH_CACHE_ID=$high_window_info_cache_id&HIGHWINDOWSIZE=$high_window_size&LOW_CACHE_ID=$low_window_info_cache_id&LOWWINDOWSIZE=$low_window_size\")>";
  }
...
#    average of window-averaged values
  ...
  my $flag = 1;
  Do_window_info_data ($chr_in, $low_window_info_cache_id, $bin_size, 
                       $CHOOSE_WAY, $flag);
  my $flag = 2;
  Do_window_info_data ($chr_in, $window_info_cache_id, $bin_size, 
                       $CHOOSE_WAY, $flag);
  my $flag = 3;
  Do_window_info_data ($chr_in, $high_window_info_cache_id, $bin_size, 
                       $CHOOSE_WAY, $flag);
...
sub Do_window_info_data {
  my ($chr_in, $window_info_cache_id, $bin_size, $CHOOSE_WAY, $flag) = @_;
  my ($n);
  ...
  while (<IN>) {
    ...
    push @start, $pos;
    if ($n % $CHOOSE_WAY == 0) {
      DoBin_detail($chr_in, $n, $bin_size, $flag);
    }
  }
  close (IN);
