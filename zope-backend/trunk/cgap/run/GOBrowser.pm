...
sub SummarizeGOForGeneSet_1 {
  ...
  my (@all_P, @all_odds_rario, @all_G_A, @BH_P);
...
#             1 - Bayesian::Bayesian(1, $b, $a, $B, $A);
  ...
  my $BH_file = tmpnam();
  open(BHOUT, ">$BH_file") || die;
  printf BHOUT join("\t", @all_P) . "\n";
  close BHOUT;
  chmod 0666, $BH_file;
 
  my $R_file = tmpnam();
  my $BH_RESULT_file = tmpnam();
 
  if (open(ROUT, ">$R_file")) {
    printf ROUT "p<-scan(\"$BH_file\")\n";
    printf ROUT "p_BH<-p.adjust(p,\"BH\")\n";
    printf ROUT "write(file = \"$BH_RESULT_file\", p_BH, sep = \"\n\", append=T)\n";
    close ROUT;
    chmod 0666, $R_file;
  }
 
  system( "/usr/local/bin/R --slave --no-save < $R_file > /dev/null 2>&1" );
  chmod 0666, $BH_RESULT_file;

  open(IN, "$BH_RESULT_file") or die "failed to open file $BH_RESULT_file\n";
  while(<IN>) {
    chop;
    push @BH_P, $_;
  }
  ...
  my %p_2_q;
  for( my $i=0; $i<@BH_P; $i++ ) {
    if( $all_odds_rario[$i] eq "HI" ) {
      $p_2_q{$all_P[$i]} = $BH_P[$i];
    }  
  }

  unlink $BH_file;
  unlink $R_file;
  unlink $BH_RESULT_file;

  if (defined %temp) {
    push @lines, join("\t", "Class", "GO Id", "GO Term",
        "Hits in list", "All annotated genes", "Fisher Exact", "False discovery rate");
    for my $P (sort numerically keys %temp) {
      for my $c ("BP", "MF", "CC") {
        if (defined $temp{$P}{$c}) {
          if( defined $p_2_q{$P} ) {
            for (my $i=0; $i<@{ $temp{$P}{$c} }; $i++ ) { 
              push @lines, ${ $temp{$P}{$c} }[$i] . "\t" . $p_2_q{$P};
            }
          }
          else {
            push @lines, @{ $temp{$P}{$c} };
