...
my $dir = "/cgap/schaefec/current/CGAP/data/";
my (%cur, %old, %diff);
...
sub GetDiff {
  ...
  my (%dev_path_ids, %dev_path_names);
  my (%pro_path_ids, %prod_path_names);
  my (%dev_paths, %prod_paths);
  my (%dev_paths, %prod_paths);
  ...
  while ($stm->fetch) {
    if ($pathway_name) {
      $prod_paths{$path_id}{"$pathway_name"} = 1;
      if ($path_id =~ /^h_/) {
  ...
  while ($stm->fetch) {
    if ($pathway_name) {
      $dev_paths{$path_id}{"$pathway_name"} = 1;
      ## if( not defined $dev_paths{$path_id}{"$pathway_name"} ) {
      if( not defined $prod_paths{$path_id} ) {
        print "***********\n";
        print "Not defined at prod: prod: $path_id, $pathway_name\n";
        ##for my $name ( keys %{$dev_paths{$path_id}} ) {
        ##  print "Not defined at dev: dev: $path_id, $name\n";
        ##}
        print "##########\n";
      }
      if ($path_id =~ /^h_/) {
...
sub InitializeDatabase {
  ...
  for my $org ("Hs", "Mm") {
    $sql =
        "select LIBRARY_ID, LIB_NAME " .
        "from $CGAP_SCHEMA.all_libraries ";
...
## Accept only categories with seq_count >= 2000
    ...
    if(not $stm) {
      ...
      while(my ($id, $name) = $stm->fetchrow_array) {
        $id2name{$id} = $name;
      }
    }
