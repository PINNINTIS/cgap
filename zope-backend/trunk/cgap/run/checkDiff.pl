...
######################################################################
sub GetDiff {
  my $file = $dir . "library_keyword.dat";
  my $file_old = $dir . "library_keyword.dat.old";
  open(C_IN, $file) or die "C";
  while(<C_IN>) {
    chop;
  ...
  close (C_IN);
  open(O_IN, $file_old) or die "O";
  while(<O_IN>) {
    chop;
    my ($id, $keyword) = split "\t", $_;
    $old{$id}{$keyword} = 1;
    if( not defined $cur{$id}{$keyword} ) {
      if( defined $tissue_types{$keyword} ) {
      ##   print "$id\t$keyword\n";
        $diff{$id}{$keyword} = 1;
      }
      ## print "$id\t$keyword\n";
  ...
  close (O_IN);
  for my $id ( keys %diff ) {
    my $line = $id . "\t" . $id2name{$id};
    ## for my $keyword ( keys %{$diff{$id}} ) {
    for my $keyword ( keys %{$old{$id}} ) {
      if( not defined $cur{$id}{$keyword} ) {
        $line = $line . "\n\t\t$keyword ## this is missing after comented the codes";
      }
      else {
        $line = $line . "\n\t$keyword";
      }
