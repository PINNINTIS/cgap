...
sub f {
  my ($x, $a, $b, $A, $B) = @_;
  my $function;
  my( $term_1, $term_2, $term_3);
  if ( defined $powers{$x}{3+$a} ) {
    $term_1 = $powers{$x}{3+$a};
  }
  else {
    $term_1 = power($x, (3+$a));
  ...
  if ( defined $powers{1-$x}{3+$b} ) {
    $term_2 = $powers{1-$x}{3+$b};
  }
  else {
    $term_2 = power((1-$x), (3+$b));
    $powers{1-$x}{3+$b} = $term_2;
  }

  if( $A != $B ) {
    if ( defined $powers{1+($A/$B-1)*$x}{$a+$b} ) {
      $term_3 = $powers{1+($A/$B-1)*$x}{$a+$b};
    }
    else {
      $term_3 = power(1+($A/$B-1)*$x,($a+$b));
      $powers{1+($A/$B-1)*$x}{$a+$b} = $term_3;
    }

    if( $term_3 == 0 ) {
      ## print "8888: $x, $a, $b, $A, $B \n"; 
      return 1;
    }
    $function = ($term_1*$term_2)/$term_3;
  }
  else {
