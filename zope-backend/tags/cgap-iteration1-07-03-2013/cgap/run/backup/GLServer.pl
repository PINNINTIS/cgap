...
sub GetSummaryTable {
  ...
  if ($partition =~ /^\s*$/) {
    SetStatus(S_NO_DATA);
    return ("<h4>$page_header</h4>\n" .
        "There are no libraries matching the query<br><br>");
  }

  if ($partition =~ /There are no libraries matching the query/) {
    SetStatus(S_NO_DATA);
    return ("<h4>$page_header</h4>\n" .
        "There are no libraries matching the query<br><br>");
  }
...
sub ListXProfiledGenes {
  ...
  if( ($filename = $cache->FindCacheFile($cache_id)) eq $CACHE_FAIL ) {
    SetStatus(S_NO_DATA);
    return "The data are missing, please go back to 'Pool A and B setup for Expression XProfiler' page and submit query again.<br><br>\n";
  }
...
## Not forkable because of race to disk cache:    "GetSummaryTable"
## Not forkable because of race to disk cache:    "ListSummarizedGenes"
## Not forkable because of cache:    "ListXProfiledGenes"
## Not forkable because of cache:    "GetXProfile"

GetBuildIDs(\%BUILDS);

InitializeDatabase();

StartServer(GL_SERVER_PORT, "GLServer");
