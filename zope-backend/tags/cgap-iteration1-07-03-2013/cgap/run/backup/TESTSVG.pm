...
sub GetmRNA_1 {
  ...
  my $graph_data = drawSVGGraph($im, $orig_x, $orig_y, $radi, \@pies);
  ...
  my ($image_cache_id);
  $image_cache_id = WriteToCache($graph_data);
    
  if (! $image_cache_id) {
    return "Cache failed";
  } 
   
  push @lines, "<center><h2>SVG TEST</h2></center><br><br><br>\n";
  push @lines,
    "<EMBED src=\"SVGImage?CACHE=$image_cache_id\" " .
    "NAME=\"SVGEmbed\" " .
...
sub drawSVGGraph {
  ...
  my $svg = SVG->new(width=>4000,height=>3000);
  my $group_0 = $svg->group(id=>'group_0');
  my $a1 = $group_0->anchor(-href=>'http://www.nih.gov');
  $a1->text(x=>120,y=>225)->cdata('SVG_TEST');
  my $group_1 = $svg->group(id=>'group_1');
  ## $group_1->rect(x=>200,y=>200,width=>40, height=>50, id=>'Rect_1', style=>{stroke=>'green',fill=>'red'});
  $group_1->rect(x=>200,y=>200,width=>40, height=>50, style=>{stroke=>'green',fill=>'red'});
  ## $group_1->rect(x=>240,y=>200,width=>40, height=>50, id=>'Rect_2', style=>{stroke=>'green',fill=>'red'});
  $group_1->rect(x=>240,y=>200,width=>40, height=>50, style=>{stroke=>'red',fill=>'green'});
  $group_1->rect(x=>280,y=>200,width=>40, height=>50, id=>'Rect_3', style=>{stroke=>'green',fill=>'red'});
  $group_1->rect(x=>320,y=>200,width=>40, height=>50, id=>'Rect_4', style=>{stroke=>'green',fill=>'red'});
  $group_1->rect(x=>3200,y=>200,width=>40, height=>50, id=>'Rect_5', style=>{stroke=>'green',fill=>'red'});
  my $group_2 = $svg->group(id=>'group_2');
  ## $group_2->circle(id=>'Circle',cx=>100,cy=>100,r=>50, style=>{stroke=>'red',fill=>'green'});
  my $group_5 = $svg->group(id=>'group_5');
  my $a5 = $group_5->anchor(-href=>'http://cgap.nci.nih.gov');
  $a5->text(x=>220,y=>190, transform=>'translate(30, 410)rotate(-90)')->cdata('SVG_TEST5');
  my $group_6 = $svg->group(id=>'group_6');
  my $a6 = $group_6->anchor(-href=>'http://cmap.nci.nih.gov');
  my $x_tr = 260 - 190;
  my $y_tr = 260 + 190;
  $a6->text(x=>260,y=>190, transform=>"translate($x_tr, $y_tr)rotate(-90)")->cdata('SVG_TEST6');
  ## $a6->text(x=>260,y=>190, transform=>'translate(275, 215)rotate(-90)')->cdata('SVG_TEST6');
  my $group_7 = $svg->group(id=>'group_7');
  my $a7 = $group_7->anchor(-href=>'http://mgc.nci.nih.gov');
  $a7->text(x=>300,y=>190, transform=>'translate(110, 490)rotate(-90)')->cdata('SVG_TEST7');

  my $tmp_str = "88888";
  $a7->text(x=>340,y=>190, style=>{'font-size'=>10,'fill'=>'rgb(51,153,255)'}, transform=>'translate(150, 530)rotate(-90)')->cdata($tmp_str);

  ## $a5->text(x=>220,y=>190)->cdata('SVG_TEST5');

  $svg->defs->rect(id=>'AAA', width=>40, height=>50, style=>{stroke=>'green',fill=>'red'});

  $svg->use(x=>200,y=>300, '-href'=>"#AAA");
  $svg->use(x=>200,y=>400, '-href'=>"#AAA");
