...
sub FixSpecChar {
  ...
  $string =~ s/\224/&ouml;/g;
  $string =~ s/\206/&aring;/g;
  $string =~ s/\202/&eacute;/g;
  $string =~ s/\204/&auml;/g;
  $string =~ s/\201/&uuml;/g;
  $string =~ s/\231/&Ouml;/g;
  $string =~ s/\217/&Aring;/g;
  $string =~ s/\264/'/g;
  $string =~ s/\351/&eacute;/g;
  $string =~ s/\233/&eacute;/g;
  $string =~ s/@/&Eacute;/g;
...
sub RefFormatRows {
  ...
  my ($count1, $count2, $count3) = CountByRefNo($$row[0]);
    ...
    push @temp,"<tr><td colspan=2 bgcolor=\"#666699\">\n".
  ...
  my (
   ...
   push @temp,
    ...
    push @temp, "<table width=500 cellspacing=3 cellpadding=3>\n".
      "<tr><td colspan=2 bgcolor=\"#666699\">\n".
...
sub RefSearch_1 {
  ...
  my $sql = BuildRefQuery($page,$author,$journal,$op,$refno,$year);
...
sub HelpLists_1 {
  ...
  my $sql = "select distinct $field from $CGAP_SCHEMA.$table";
...
sub GetBacByChromosome_1
{
  ...
  push @output, "used for clone placement.<br>&nbsp;&nbsp; " .
                "<IMG src=\"" . IMG_DIR . "/L_3.gif\"> " .
                " BAC-end sequence aligned in the plus orientation " .
                " <br>&nbsp;&nbsp; " .
                "<IMG src=\"" . IMG_DIR . "/L_2.gif\"> " .
                " BAC-end sequence aligned in the minus orientation " .
                " <br>&nbsp;&nbsp; " .
                "<IMG src=\"" . IMG_DIR . "/M_2.gif\"> " .
                " insert sequence<br>&nbsp;&nbsp; " .
                "<IMG src=\"" . IMG_DIR . "/M_1.gif\"> " .
                " STS aligned<br>&nbsp;&nbsp; " .
                "<IMG src=\"" . IMG_DIR . "/LL_1.gif\"> " .
    ...
    if( $BOUNDARY_CATEGORY ne "" ) {
      ...
      for ( my $i=0; $i<@tmp; $i++ ) {
        if( $tmp[$i] =~ /^LL_/ or $tmp[$i] =~ /^RR_/ ) {
          $graph = $graph . 
              "<img src=\"" .  IMG_DIR . "/$tmp[$i]\" width=3 height=25 >";
        } 
        elsif( $tmp[$i] =~ /^L_/ or $tmp[$i] =~ /^R_/ ) {
          $graph = $graph . 
              "<img src=\"" .  IMG_DIR . "/$tmp[$i]\" width=14 height=25 >";
        }
        elsif( $tmp[$i] =~ /^M_/ ) {
          $graph = $graph . 
              "<img src=\"" .  IMG_DIR . "/$tmp[$i]\" width=16 height=25 >";
        }
      }
      $graph = $graph . "</TD>"; 
    }
