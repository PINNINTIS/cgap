...
sub DrawGrid {
  ...
  for (my $i = 0; $i <= $NUM_TICKS; $i++) {
    ...
    push @{ $image_map },
        ...
        "href=\"GEQuery?ORG=$org&EFOLD=$exp_fold&ZFOLD=2&ZLEVEL=" . int($zlevel+1) .
        "&ZPOINT=$scaled_tick" .
        "&CHR=$chr&SRC=$src&TISSUE=$tissue&FILTER=$filter\" " .
        "alt=\"zoom in\">";
  }
...
sub GetCytoBandBounds {
  my ($db, $org, $chr, $start, $end, $band2pos, $band2stain) = @_;
...
sub GetGenesInWindow {
  ...
  $sql =
      ...
      "from $CGAP_SCHEMA.ucsc_mrna r, $CGAP_SCHEMA." .
      ($org eq "Hs" ? "hs_cluster" : "mm_cluster") . " c " .
      "where r.CHROMOSOME = '$chr' and r.ORGANISM = '$org' " .
...
sub GenomeExpression_1 {
  ...
  $scaled_chr_length = sprintf("%d", GetChrLength($db, $chr) / $BP_SCALE);

  if ($scaled_start < 0 || $scaled_start > $scaled_end) {
    print STDERR "incompatible start = $scaled_start and end = $scaled_end\n";
    exit();
  }
  if ($zoomfold < 1) {
    $zoomfold = 1;
  ...
  $zoomscale  = $zoomfold**$zoomlevel;

  if ($scaled_zoompoint) {

    my ($zoomstart_bp, $zoomend_bp, $usable_axis_in_scaled_bp);

    if ($scaled_start && $scaled_zoompoint < $scaled_start) {
      print STDERR "incompatible start = $scaled_start and " .
          "zoompoint = $scaled_zoompoint\n";
      exit();
    } elsif ($scaled_end && $scaled_zoompoint > $scaled_end) {
      print STDERR "incompatible end = $scaled_end and " .
          "zoompoint = $scaled_zoompoint\n";
      exit();
    }

    ## if ok, let zoompoint take precedence

    $usable_axis_in_scaled_bp = $ZOOMED_AXIS_LENGTH / $zoomscale;
    $zoomstart_bp = sprintf("%d",
        $scaled_zoompoint - $usable_axis_in_scaled_bp / 2);
    $zoomend_bp   = sprintf("%d",
        $scaled_zoompoint + $usable_axis_in_scaled_bp / 2);
    if ($zoomstart_bp < 0) {
      $scaled_start = 0;
    } else {
      $scaled_start = $zoomstart_bp;
    }
    if ($zoomend_bp > $scaled_chr_length) {
      $scaled_end = $scaled_chr_length;
    } else {
      $scaled_end = $zoomend_bp;
    }
  } else {
#    $zoomfold  = 1;
#    $zoomlevel = 1;
  ...
  GetVNTotals($db, $org, $tissue, $data_src);

  if (defined $tiss2code{$tissue}) {
    $tissue_code = $tiss2code{$tissue};
  } else {
    print "No data for tissue $tissue\n";
    exit();
  }

  if ((not defined $vn_seq_count{"$tissue_code,1"}) ||
      ($vn_seq_count{"$tissue_code,1"} < 2000)) {
    print "No data for $tissue cancer\n";
    exit();
  }
  if ((not defined $vn_seq_count{"$tissue_code,2"}) ||
      ($vn_seq_count{"$tissue_code,2"} < 2000)) {
    print "No data for normal $tissue\n";
    exit();
  }

  GetGeneExpressionData($db, $org, $chr, $scaled_start * $BP_SCALE,
      $scaled_end * $BP_SCALE, $tissue_code,
      $data_src, $filter, $exp_fold, \@expr_info, \%gene_sym);

  if( $org eq "Hs" ) {
    GetCytoBandBounds($db, $org, $chr, $scaled_start * $BP_SCALE,
      $scaled_end * $BP_SCALE, \%band2pos, \%band2stain);
  }
  ...
  push @image_map, "<map name=\"chrmap\">";

  if( $org eq "Hs" ) {
    DrawCytoBands($im, $scaled_start, $scaled_end, \%band2pos, \%band2stain,
      $zoomscale, \@image_map);
  }
...
#print STDERR "start = $start, end = $end\n";
  ...
  $tissue =~ s/ /+/g;
  DrawGrid($im, $scaled_start, $scaled_end, $zoomscale,
      $org, $chr, $tissue, $data_src, $filter, $exp_fold,
      $zoomfold, $zoomlevel, \@image_map);

  DrawGeneBars($im, $scaled_start, $zoomlevel,
      $zoomscale, $org, \@expr_info, \%gene_sym, \@image_map);

  push @image_map, "</map>";

  if (GD->require_version() > 1.19) {
    $image_cache_id = WriteGEToCache($im->png);
  } else {
    $image_cache_id = WriteGEToCache($im->gif);
  }
  ...
  my @lines;
  push @lines,
      "<image src=\"GEImage?CACHE=$image_cache_id\" alt=\"Genome Expression Image\" " .
      "border=0 " .
      "usemap=\"#chrmap\">";
