...
##   print GetLibrary_1($base, $page, $org1, $scope1, $title1, $type1, $tissue_types[$i], $hist1,
##       $prot1, $keys1, $sort1);
## }
## $org1 = "Mm";
## for ( my $i=0; $i<@tissue_types; $i++ ) {
...
my $dir = "/cgap/webcontent/LINUX/staging/CGAP/run/";
my (%dev,%prod);

Getcgdev();
Getcgprod();
my $hs_diff_out = $dir . "hs_prod_only.txt";
my $mm_diff_out = $dir . "mm_prod_only.txt";
my $out = $dir . "no_return_for_both.txt";
open(HS_PROD, ">$hs_diff_out") or die "HS PROD";
open(MM_PROD, ">$mm_diff_out") or die "MM PROD";
open(BOTH, ">$out") or die "BOTH";
Diff();
close (HS_PROD);
close (MM_PROD);
close (BOTH);
...
######################################################################
sub Getcgdev {
  my $file = $dir . "cgdev_result_all";
  open(DEV_IN, $file) or die "Dev";
  while(<DEV_IN>) {
    chop;
    my ($org, $tiss, $name) = split "\t", $_;
    $dev{$org}{$tiss}{$name} = 1;
  } 
  close (DEV_IN);
}
...
######################################################################
sub Getcgprod {
  my $file = $dir . "cgprod_result_all";
  open(PROD_IN, $file) or die "PROD";
  while(<PROD_IN>) {
    chop;
    my ($org, $tiss, $name) = split "\t", $_;
    $prod{$org}{$tiss}{$name} = 1;
  } 
  close (PROD_IN);
}
...
sub Diff {
  for ( my $i=0; $i<@tissue_types; $i++ ) {
    if( not defined $prod{"Hs"}{$tissue_types[$i]} and not defined $prod{"Mm"}{$tissue_types[$i]} ) {
      print BOTH $tissue_types[$i] . "\n";
    }
    else {
      if ( defined $prod{"Hs"}{$tissue_types[$i]} ) {
        for my $name ( keys %{$prod{"Hs"}{$tissue_types[$i]}} ) {
          if( not defined $dev{"Hs"}{$tissue_types[$i]}{$name} ) {
            print HS_PROD $tissue_types[$i] . "\t" . $name . "\n";
          }
        }
      }
      if ( defined $prod{"Mm"}{$tissue_types[$i]} ) {
        for my $name ( keys %{$prod{"Mm"}{$tissue_types[$i]}} ) {
          if( not defined $dev{"Mm"}{$tissue_types[$i]}{$name} ) {
            print MM_PROD $tissue_types[$i] . "\t" . $name . "\n";
          }
        }
      }
    }
...
sub InitializeDatabase {
  ...
  for my $org ("Hs", "Mm") {
    $sql =
        "select v.source, s.tissue_code, s.tissue_name, v.histology_code, " .
...
## Accept only categories with seq_count >= 2000
    ...
    if(not $stm) {
      ...
      while(($src, $tissue_code, $tissue, $histology_code,
          $lib_count, $seq_count) = $stm->fetchrow_array) {
        $vn_lib_count{$org}{$src}{"$tissue_code,$histology_code"} = $lib_count;
        $vn_seq_count{$org}{$src}{"$tissue_code,$histology_code"} = $seq_count;
...
sub FormatOneLib {
  ...
  if ($what eq "HTML") {
    ...
    ## $s = "$title\t$the_tiss\t$the_hist\t$the_prep\t$the_prot\t$keys";
    $s = "$org\t$title\t$the_tiss";
  }
  return $s;
}
...
sub GetLibrary_1 {
  ...
  ##
    ...
    return FormatLibs($page, $org1, $cmd, $page_header, \@info);
